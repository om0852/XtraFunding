...
export default function InvestorLayout({
  ...
  const currentModel = searchParams ? searchParams.get('model') : null;
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/campaign?search=${encodeURIComponent(searchQuery)}`);
    }
  };
        ...
        <header className={styles.topbar}>
          ...
          {pathname.includes('/investor') || pathname.includes('/campaign') ? (
            <form className={styles.searchContainer} onSubmit={handleSearch}>
              <input 
                ...
                placeholder="Search startups, sectors, deals..." 
                value={searchQuery}
          ...
          <div className={styles.topbarActions}>
            ...
            <div className={styles.topAvatar} onClick={() => setIsProfileOpen(!isProfileOpen)}>
              {user ? getInitials(user.name) : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>}
            </div>
            ...
            {isProfileOpen && (
              <div className={styles.profileDropdown}>
                <div className={styles.dropdownHeader}>
                  <div className={styles.dropdownAvatar}>{user ? getInitials(user.name) : <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>}</div>
